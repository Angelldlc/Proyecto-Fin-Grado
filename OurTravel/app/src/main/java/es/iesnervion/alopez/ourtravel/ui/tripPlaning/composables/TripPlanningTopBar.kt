...
fun TripPlanningTopBar(
    ...
    LaunchedEffect(nameState){
        if(nameState != trip.name){
            updateTripPlanning(trip.id.toString(), nameState.toString(), trip.startDate!!, trip.endDate!!, trip.totalCost ?: 0, try { destinations[0].cityPhoto } catch (e: Exception) { "" })
...
@Composable
fun TripPlanningTopBarActions(openDialog: () -> Unit, trip: TripPlanning, destinations: Destinations){
...
fun generatePDF(context: Context, trip: TripPlanning, destinations: Destinations) {
    // Crear el archivo PDF usando iText
    ...
    if (ContextCompat.checkSelfPermission(context, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
        ActivityCompat.requestPermissions(context as Activity, arrayOf(Manifest.permission.WRITE_EXTERNAL_STORAGE), 100)
    } else {
...
//            document.add(Image.getInstance(trip.photo))
            ...
            //TODO Terminar documento con los destinos del viaje
//            document.close()
        } catch (e: Exception){
